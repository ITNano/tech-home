...
class PocketSphinxSTT(AbstractSTTEngine):
    ...
    def __init__(self, vocabulary, hmm_dir="/usr/local/share/" +
        ...
        config.set_string('-lm', vocabulary.decoder_kwargs.get('lm'))
        config.set_string('-dict', vocabulary.decoder_kwargs.get('dict')) 
    ...
    def transcribe(self, fp):
        ...
        if result != None:
            ...
            self._logger.info('Transcribed: %r (probability %d)', transcribed, result.best_score)
        else:
            transcribed = ''
        return transcribed
